...
The build environment is based on a tutorial by Tyler McGinnis:

https://tylermcginnis.com/react-js-tutorial-1-5-utilizing-webpack-and-babel-to-build-a-react-js-app-5f804d729d3b#.oontdfzsy>

**note**: We explored using generator tools, like `create-react-app`, but
found them too limiting (and complex under-the-hood).
...
The principle action creator is `removeItem`; it removes an item from the
...
controller-view component. This is possible because `removeItem` returns
a promise.

* The call to the action creators `resetAddItemPartForm`, `resetUpdateItemForm`,
and `resetRemovePartError` reset the respective forms (removes errors).
* The call to the action creator `setBlocking` manages the effect of popping up a
spinner for the duration of the removal operation.
* The call to `push` manages the effect of routing back to the list of
items on success.
...
by the action creator `removeItem`. The problem with this approach is that
